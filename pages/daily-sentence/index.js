...
Page({
  ...
  onShow() {
    const targetId = wx.getStorageSync('kr_daily_sentence_target_id');
    const targetTs = wx.getStorageSync('kr_daily_sentence_target_ts');
    const shouldForceLatest = wx.getStorageSync('kr_daily_sentence_force_latest');

    if (targetId || targetTs) {
        try { wx.removeStorageSync('kr_daily_sentence_target_id'); } catch (e) {}
        try { wx.removeStorageSync('kr_daily_sentence_target_ts'); } catch (e) {}
        this.loadTarget(targetId, targetTs);
        return;
    }

    if (shouldForceLatest) {
        try { wx.removeStorageSync('kr_daily_sentence_force_latest'); } catch (e) {}
  ...
  async loadTarget(targetId, targetTs) {
    ...
    try {
        if (targetId) {
            const cacheKey = `kr_daily_sentence_cache_id_${targetId}`;
            loaded = readCache(cacheKey, ttlMs);
            if (!loaded) {
                const callRes = await callDailySentence({ id: targetId });
                const result = callRes && callRes.result ? callRes.result : null;
                const list = result && Array.isArray(result.data) ? result.data : [];
                loaded = list[0] || null;
                if (loaded) writeCache(cacheKey, loaded);
            }
        }
        
        if (!loaded && targetTs) {
             const n = Number(targetTs);
    ...
    if (!loaded) {
        ...
        wx.showToast({ title: '未找到对应内容', icon: 'none' });
        return;
    }

    const picked = loaded;
    const useTs = targetTs ? Number(targetTs) : (picked.timestamp ? Number(picked.timestamp) : null);
    const normalized = normalizeSentence(picked, useTs);
    
    const hasMeaningful = !!(normalized.sentence && normalized.sentence.source) || (Array.isArray(normalized.translations) && normalized.translations.length > 0);
    if (hasMeaningful) {
      addDailySentenceHistoryEntry({ ...normalized.sentence, timestamp: normalized.resolvedTs });
    }
